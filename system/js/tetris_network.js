...
class TetrisNetwork {
     ...
     */
    connect() {
        // 公開時は実際のドメインまたはIPに書き換え
        this.socket = io('http://YOUR_UBUNTU_IP:3000', {
            auth: {
    ...
    setupEventListeners() {
        ...
        this.socket.on('opponent-update', (data) => {
            // data: { userId, grid, nextPiece, holdPiece, isGameOver }
    ...
    drawOpponentGrid(canvas, grid) {
        ...
        grid.forEach((row, y) => {
            row.forEach((cell, x) => {
                if (cell) {
                    ctx.fillStyle = cell === 1 ? '#555' : cell; // 簡易描画
